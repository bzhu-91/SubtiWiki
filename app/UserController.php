...
class UserController extends \Kiwi\Controller {
	/**
	 * get method
	...
	protected function list ($input, $accept) {
		...
		switch ($accept) {
			case JSON:
				$this->respond(\Kiwi\Utility::arrayColumns($users, ["name", "description"]), 200, JSON);
				break;
			case HTML:
			case HTML_PARTIAL:
	...
	public function password ($input, $accept, $method) {
		if ($method == "POST") {
			...
			if ($shouldSendEmail) {
				...
				if (\Kiwi\Utility::sendEmail($user->email, $user->name, "Reset password for your ".$GLOBALS["SITE_NAME"]." account", $body)) {
	...
	public function login ($input, $accept, $method) {
		switch ($method) {
			case 'GET':
				switch ($accept) {
					case HTML:
						$view = \Kiwi\View::loadFile("layout3.tpl");
						$view->set([
						...
						break;
					case HTML_PARTIAL:
						$view = \Kiwi\View::loadFile("user.login.tpl");
						$this->respond($view, 200, HTML);
						break;
					case JSON:
		...
		switch($accept) {
			...
			case JSON:
				...
				if ($token) {
					...
					if ($input["email"] && \Kiwi\Utility::validateEmailAddressAddressAddressAddressAddressAddress($input["email"])) {
				...
				switch ($accept) {
					case JSON:
						...
						if ($invitation->insert()) {
							if ($sendEmail) {
								\Kiwi\Utility::sendEmail($email, $name, "Your SubtiWiki invitation token", $body);
